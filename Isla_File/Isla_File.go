...
func encryptList(plainText string, key []byte) (string, error) {
	...
	// Agregar padding al plaintext y complementar los bytes necesarios
...
func (s *server) NotifyCompletion(ctx context.Context, info *pb.CompletionInfo) (*pb.Ack, error) {
	...
	s.ps = sacrificio
	log.Printf("Isla_File recibió notificación de finalización. PS: %.2f", s.ps)
	...
	if len(selectedDigimons) == 0 {
		log.Println("Isla_File no tiene Digimons para sacrificar.")
	...
	go func() {
		...
		for range ticker.C {
			if len(s.digimons) == 0 {
				log.Println("Isla_File se quedó sin Digimons para enviar.")
				return
			}
	...
	return &pb.Ack{Success: true, Message: "Isla_File: Sacrificio procesado y resultados enviados"}, nil
}
...
func sendDigimonBatch(digimons []Digimon, key []byte, sacri float64) {
	...
	for _, digimon := range digimons {
		...
		fmt.Printf("Isla_File: Estado enviado: %s, %s\n", n, e)
	}
	...
	fmt.Println("Isla_File: Mensaje encriptado enviado a PrimaryNode:", encrypted)
...
func sendResults(serverAddr, resultados string) error {
	...
	if err != nil {
		log.Fatalf("Isla_File: sin conexion con PrimaryNode, finalizando ejecucion")
	}
...
func main() {
	...
	lis, err := net.Listen("tcp", "0.0.0.0:50051") // Puerto para Isla_File
	if err != nil {
	...
	log.Println("Isla_File: Isla_File está escuchando en el puerto 50051")
