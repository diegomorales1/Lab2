...
func (s *server) NotifyCompletion(ctx context.Context, info *pb.CompletionInfo) (*pb.Ack, error) {
	...
	s.ps = sacrificio
	log.Printf("ContinenteServer: ContinenteServer recibió notificación de finalización. PS: %.2f", s.ps)
	...
	if len(selectedDigimons) == 0 {
		log.Println("ContinenteServer no tiene Digimons para sacrificar.")
		return &pb.Ack{Success: false, Message: "No hay Digimons para sacrificar"}, nil
	}
	...
	go func() {
		...
		for range ticker.C {
			if len(s.digimons) == 0 {
				log.Println("ContinenteServer se quedó sin Digimons para enviar.")
				return
			}
	...
	return &pb.Ack{Success: true, Message: "ContinenteServer: Sacrificio procesado y resultados enviados"}, nil
}
...
func sendDigimonBatch(digimons []Digimon, key []byte, sacri float64) {
	...
	for _, digimon := range digimons {
		...
		fmt.Printf("ContinenteServer: Estado enviado: %s, %s\n", n, e)
	}
	...
	fmt.Println("ContinenteServer: Mensaje encriptado enviado a PrimaryNode:", encrypted)
...
func sendResults(serverAddr, resultados string) error {
	...
	if err != nil {
		log.Fatalf("ContinenteServer: sin conexion con PrimaryNode, finalizando ejecucion")
	}
...
func main() {
	...
	lis, err := net.Listen("tcp", "0.0.0.0:50053") // Puerto para ContinenteServer
	if err != nil {
	...
	log.Println("ContinenteServer: ContinenteServer está escuchando en el puerto 50053")
